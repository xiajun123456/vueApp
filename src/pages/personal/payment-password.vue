<template>
    <div class="payment-container">
        <div class="global-header header-comm">
            <div class="go-before" @click="$router.go(-1)">
                <span class="glyphicon glyphicon-menu-left"></span>
            </div>
            <span>设置支付密码</span>
        </div>
        <div class="input-container">
            <mt-field label="手机号:" placeholder="手机号" type="tel" v-model="payment.mobile"></mt-field>
            <mt-field label="支付密码:" placeholder="输入密码" type="password" v-model="payment.password"></mt-field>
            <mt-field label="确认密码:" placeholder="确认密码" type="password" v-model="payment.newPassword"></mt-field>
            <mt-field label="验证码:" placeholder="验证码"  v-model="payment.verificationCode"></mt-field>
            <div class="verification-code">
                <button class="btn"
                        :class="{'btn-verification':!code,'btn-code':code}"
                        @click="getVerification">
                    <span v-if="!code">{{getCodeTxt}}</span>
                    <span v-if="code">{{times}} s</span>
                </button>
            </div>
        </div>
        <div class="technical-support">
            <img src="data:image/png;base64,"
                 alt="">
            <p>技术支持 古美术</p>
        </div>
        <div class="save-container">
            <button class="btn save-btn" @click="save">保存</button>
        </div>
    </div>
</template>
<script type="text/ecmascript-6">
    import prompt from '../../shared/services/promptServices';
    import request from '../../shared/request/asyncRequest'
    export default {
        data(){
            return {
                payment:{
                    mobile:'',
                    password:'',
                    newPassword:'',
                    verificationCode:''
                },
                code: false,
                times: 60,
                getCodeTxt: '获取验证码'
            }
        },
        methods:{
            getVerification(){
                if (this.code) {
                    return
                }
                let mobileReg = /^1[34578]\d{9}$/, timers = null
                if (!this.payment.mobile || !mobileReg.test(this.payment.mobile)) {
                    prompt.alert('电话号码为空或者格式不正确!');
                    return
                }
                request.enroll.getCode({mobile: this.payment.mobile}).then(data => {
                    this.code = true;
                    this.times = 60;
                    prompt.alert('发送成功!');
                    timers = setInterval(() => {
                        this.times--;
                        if (this.times == 0) {
                            this.code = false;
                            clearInterval(timers);
                            timers = null
                        }
                    }, 1000)
                }).catch((err) => {
                    prompt.alert(err.message || '服务器错误!');
                })
            },
            save(){
                let mobileReg = /^1[34578]\d{9}$/, timers = null
                if (!this.payment.mobile || !mobileReg.test(this.payment.mobile)) {
                    prompt.alert('电话号码为空或者格式不正确!');
                    return
                }
                if (!this.payment.password) {
                    prompt.alert('密码不能为空!');
                    return
                }
                if (this.payment.password != this.payment.newPassword) {
                    prompt.alert('2次密码输入不一致!');
                    return
                }
                if (!this.payment.verificationCode) {
                    prompt.alert('验证码不能为空!');
                    return
                }
                request.personal.setPaymentPassword(this.payment).then(data => {
                    prompt.alert(data.message);
                    this.$router.push({path:'/myData'})
                }).catch(err => {
                    prompt.alert(err.message || '服务器错误');
                })
            }
        },
        mounted(){
            var _vm = this;
            this.$store.state.isLogin = true;
        }
    }
</script>
<style type="text/scss" lang="scss">
    .save-container{
        position: fixed;
        bottom: 0;
        left: 0;
        right: 0;
        text-align: center;
        .save-btn{
            background: #ee3333;
            height: 42px;
            color: #fff;
            font-size: 1.1em;
            width: 150px;
            margin-bottom: 10px;
        }
    }
    .payment-container{
        .input-container{
            position: relative;
            .verification-code{
                position: absolute;
                right: 0;
                bottom: 10px;
                .btn-code {
                    background: #dddddd;
                    color: #fff;
                }
                .btn-verification{
                    background: #e33;
                    color: #fff;

                }
            }
        }
        .technical-support{
            position: absolute;
            bottom: 60px;
            left: 0;
            right: 0;
            text-align: center;
            color: #888;
            img{
                width: 45px;
                height: 45px;
            }
        }

    }
</style>