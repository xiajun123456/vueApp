<template>
    <div class="add-bank-container">
        <div class="global-header header-comm">
            <div class="go-before" @click="$router.go(-1)">
                <span class="glyphicon glyphicon-menu-left"></span>
            </div>
            <span>添加银行卡</span>
        </div>
        <div class="address">
            <div class="address-txt">
                地区:
            </div>
            <div class="address-select">
                <address-selector v-model="area"></address-selector>
            </div>
        </div>
        <div class="address">
            <div class="address-txt">
                银行:
            </div>
            <div class="address-select">
                <select class="bank-select" v-model="add.bank">
                    <option value="">请选择</option>
                    <option :value="bank.bankId" v-for="bank in bankList">{{bank.bankName}}</option>
                </select>
            </div>
        </div>
        <mt-field label="支行:" placeholder="请输入支行信息" v-model="add.bankInformation"></mt-field>
        <mt-field label="卡号:" placeholder="请输入卡号" v-model="add.carNum"></mt-field>
        <mt-field label="持卡人:" placeholder="请输入持卡人" v-model="add.cardHolder"></mt-field>
        <div class="save-container">
            <button class="btn save-btn" @click="saveBank">保存</button>
        </div>
        <div class="technical-support">
            <img src="data:image/png;base64,"
                 alt="">
            <p>技术支持 古美术</p>
        </div>
    </div>
</template>
<style type="text/scss" lang="scss">
    .add-bank-container {
        .address {
            padding: 0 10px;
            height: 48px;
            position: relative;
            &:after {
                content: '';
                display: block;
                clear: both;
            }
            .address-txt {
                float: left;
                width: 105px;
                line-height: 48px;
                font-size: 16px;
            }
            .address-select {
                position: absolute;
                left: 105px;
                right: 10px;
                top: 0;
                bottom: 0;
            }
            .bank-select {
                width: 100%;
                height: 48px;
                border: none;
                outline: none;
            }
        }
    }
</style>
<script type="text/ecmascript-6">
    import addressSelector from '../../shared/components/addressSelector.vue';
    import request from '../../shared/request/asyncRequest';
    import prompt from '../../shared/services/promptServices';
    import _ from 'lodash'
    export default {
        data(){
            return {
                area: {},
                add: {
                    bank: '',
                    bankInformation: '',
                    carNum: '',
                    cardHolder: ''
                },
                bankList: []
            }
        },
        methods: {
            saveBank(){
                var reg = /^(\d{16}|\d{19})$/;
                if (!this.area.area) {
                    prompt.alert('地区不能为空!');
                    return
                }
                if (!this.add.bank) {
                    prompt.alert('请选择银行!');
                    return
                }
                if (!this.add.bankInformation) {
                    prompt.alert('支行信息不能为空');
                    return
                }
                if (!reg.test(this.add.carNum)) {
                    prompt.alert('卡号输入错误或为空');
                    return
                }
                if(!this.add.cardHolder){
                    prompt.alert('持卡人不能为空');
                    return
                }
                var data = _.defaults(this.add,{});
                data.area = this.area;
                request.personal.addBank(data).then(data => {
                    prompt.alert(data.message);
                    this.$router.push({path:'/bank'})
                }).catch(err => {
                    prompt.alert(err.message || '服务器错误!');
                })
            }
        },
        components: {
            addressSelector
        },
        mounted(){
            this.$store.state.isLogin = true;
            request.personal.geAllbBank({}).then(data => {
                this.bankList = data.rows
            }).catch(err => {
                prompt.alert(err.message || '服务器错误!');
            })
        }
    }
</script>