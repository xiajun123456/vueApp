<template>
    <div class="preview-container">
        <div class="global-header header-comm">
            <div class="go-before"
                 @click="$router.go(-1)">
                <span class="glyphicon glyphicon-menu-left"></span>
            </div>
            <span>预览列表</span>
        </div>
        <div class="preview-times">
            {{time}}
        </div>
        <div class="preview-list">
            <ul>
                <li class="preview-item"
                    v-for="list in commodityList"
                    @click="goDetails(list)">
                    <span class="glyphicon glyphicon-menu-right item-icon"></span>
                    <div class="img-c">
                        <img :src="list.url" alt="">
                    </div>
                    <ul class="preview-detail">
                        <li class="preview-detail-item preview-detail-header">
                            {{list.name}}
                        </li>
                        <li class="preview-detail-item">
                            <span class="item-title">起拍价格:</span>￥{{list.startingPrice}}
                        </li>
                        <li class="preview-detail-item">
                            <span class="item-title">当前价格:</span>{{list.currentPrice?'￥'+list.currentPrice :'--'}}
                        </li>
                        <li class="preview-detail-item">
                            <span class="item-title">开拍时间:</span>{{list.startTimes}}
                        </li>

                    </ul>
                </li>
            </ul>
        </div>
        <div class="technical-support">
            <img src="data:image/png;base64,"
                 alt="">
            <p>
                技术支持 古美术
            </p>
        </div>
    </div>
</template>
<script type="text/ecmascript-6">
    import request from '../../shared/request/asyncRequest';
    import prompt from '../../shared/services/promptServices';
    export default {
        data(){
            return {
                time: '',
                commodityList: []
            }
        },
        methods: {
            goDetails(list){
                this.$store.state.holdingDetail = list;
                this.$router.push({name: 'holdingDetail', params: {id: list.id}})
            }
        },
        mounted(){
            this.$store.state.isLogin = true;
            let _vm = this;
            request.dashboard.getCommodityList({id: this.$route.params.typeId}).then(data => {
                _vm.time = data.times;
                _vm.commodityList = data.rows
            }).catch(err => {
                prompt.alert(err.message || '服务器错误!');
            })
        }
    }
</script>
<style type="text/scss" lang="scss">
    .preview-container {
        .technical-support {
            position: relative;
            bottom: 0;
            margin-top: 20px;
        }
        .preview-times {
            margin: 0 -15px;
            line-height: 42px;
            padding-left: .8em;
            border-bottom: 1px solid #dddddd;
            font-size: 1.2em;
            color: #333;
        }
        .preview-list {
            .preview-item {
                border-bottom: 1px solid #ddd;
                padding: .7em 0;
                position: relative;
                height: 120px;
                .item-icon {
                    position: absolute;
                    top: 12px;
                    right: -9px;
                    font-size: 1em;
                    color: #888;
                }
                .img-c {
                    img {
                        width: 6em;
                        height: 6em;
                        border-radius: 3px;
                    }
                }
                .preview-detail {
                    position: absolute;
                    left: 6em;
                    right: 0;
                    top: .5em;
                    bottom: 1em;
                    padding-left: 1em;
                    .preview-detail-item {
                        margin-bottom: .3em;
                        color: #ee3333;
                        .item-title {
                            color: #9e9e9e;
                            margin-right: .3em;
                        }
                    }
                    .preview-detail-header {
                        color: #111;
                        position: relative;
                        white-space: nowrap;
                        overflow: hidden;
                        text-overflow: ellipsis;
                        font-size: 1.2em;
                        padding-right: .3em;
                    }
                }
            }
        }

    }
</style>